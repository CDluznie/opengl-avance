...
vec3 blinnPhong(vec3 lightDir, vec3 lightIntensity) {
	...
	return lightIntensity*(Kd*(max(0., dot(wi,vViewSpaceNormal))) + Ks*(pow(max(0., dot(halfVector,vViewSpaceNormal)), shininess)));
}
...
void main() {
	...
	// Ambiant light add once
	
	vec3 Ka = texture(uKaSampler, vTexCoords).xyz * uKa;
	
	fColor = CumulatedDirectionalLight + CumulatedPointLight + Ka;
